...
// This file is not compiled. It simply contains class and function
// definitions needed by the QKXTM functions. 
...
void calcMG_threepTwop_EvenOdd(void **gauge_APE, void **gauge, 
  ...
  if(param->solve_type != QUDA_DIRECT_PC_SOLVE) 
    errorQuda("This function works only with Direct solve and even odd preconditioning");
  ...
  if(corrThp_local == NULL || 
     ...
     corrThp_oneD == NULL) 
    errorQuda("Cannot allocate memory for Three-point function write Buffers.");
  ...
  if(corrMesons == NULL || 
     corrBaryons == NULL) 
    errorQuda("Cannot allocate memory for 2-point function write Buffers.");
  ...
  if( CorrFileFormat==HDF5_FORM ){
    if( (Thrp_local_HDF5   = 
	 (double*) malloc(2*16*alloc_size*2*info.Ntsink*NprojMax*sizeof(double)))==NULL ) 
      errorQuda("Cannot allocate memory for Thrp_local_HDF5.\n");
    if( (Thrp_noether_HDF5 = 
	 (double*) malloc(2* 4*alloc_size*2*info.Ntsink*NprojMax*sizeof(double)))==NULL ) 
      errorQuda("Cannot allocate memory for Thrp_noether_HDF5.\n");
    ...
    if( (Thrp_oneD_HDF5 = (double**) malloc(4*sizeof(double*))) == NULL ) 
      errorQuda("Cannot allocate memory for Thrp_oneD_HDF5.\n");
    for(int mu=0;mu<4;mu++){
      if( (Thrp_oneD_HDF5[mu] = 
	   (double*) malloc(2*16*alloc_size*2*info.Ntsink*NprojMax*sizeof(double)))==NULL ) 
	errorQuda("Cannot allocate memory for Thrp_oned_HDF5[%d].\n",mu);
      
      memset(Thrp_oneD_HDF5[mu], 0, 2*16*alloc_size*2*info.Ntsink*NprojMax*sizeof(double));
    }

    if( (Twop_baryons_HDF5 = (double*) malloc(2*16*alloc_size*2*N_BARYONS*sizeof(double)))==NULL ) 
      errorQuda("Cannot allocate memory for Twop_baryons_HDF5.\n");
    if( (Twop_mesons_HDF5  = (double*) malloc(2   *alloc_size*2*N_MESONS *sizeof(double)))==NULL ) 
      errorQuda("Cannot allocate memory for Twop_mesons_HDF5.\n");
...
void calcMG_loop_wOneD_TSM_EvenOdd(void **gaugeToPlaquette, 
  ...
  if(param->inv_type != QUDA_GCR_INVERTER) 
    errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: This function works only with GCR method");
  
  if(param->gamma_basis != QUDA_UKQCD_GAMMA_BASIS) 
    errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: This function works only with ukqcd gamma basis\n");
  if(param->dirac_order != QUDA_DIRAC_ORDER) 
    errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: This function works only with color-inside-spin\n");
  
  if( info.isEven && (param->matpc_type != QUDA_MATPC_EVEN_EVEN) ){
    errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: Inconsistency between operator types!");
  }
  ...
  if(info.isEven){
    printfQuda("calcMG_loop_wOneD_TSM_EvenOdd: Solving for the Even-Even operator\n");
    flag_eo = true;
  }
  else{
    printfQuda("calcMG_loop_wOneD_TSM_EvenOdd: Solving for the Odd-Odd operator\n");
    flag_eo = false;
  }

  if (!initialized) 
    errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: QUDA not initialized");
  pushVerbosity(param->verbosity);
  if (getVerbosity() >= QUDA_DEBUG_VERBOSE) 
    printQudaInvertParam(param);
  ...
  // gen-ultra_local
  loopInfo.loop_type[1] = "dOp";    loopInfo.loop_oneD[1] = false;   
  // std-one_derivative
  ...
  if((cudaHostAlloc(&std_uloc, sizeof(double)*2*16*GK_localVolume, 
    ...
    errorQuda("Error allocating memory std_uloc\n");
  if((cudaHostAlloc(&gen_uloc, sizeof(double)*2*16*GK_localVolume, 
		    cudaHostAllocMapped)) != cudaSuccess)
    errorQuda("Error allocating memory gen_uloc\n");
  if((cudaHostAlloc(&tmp_loop, sizeof(double)*2*16*GK_localVolume, 
		    cudaHostAllocMapped)) != cudaSuccess)
    errorQuda("Error allocating memory tmp_loop\n");
  ...
  if(gen_oneD == NULL)errorQuda("Error allocating memory gen_oneD higher level\n");
  if(std_oneD == NULL)errorQuda("Error allocating memory std_oneD higher level\n");
  if(std_csvC == NULL)errorQuda("Error allocating memory std_csvC higher level\n");
  if(gen_csvC == NULL)errorQuda("Error allocating memory gen_csvC higher level\n");
  cudaDeviceSynchronize();

  for(int mu = 0; mu < 4 ; mu++){
    if((cudaHostAlloc(&(std_oneD[mu]), sizeof(double)*2*16*GK_localVolume, 
		      cudaHostAllocMapped)) != cudaSuccess)
      errorQuda("Error allocating memory std_oneD\n");
    if((cudaHostAlloc(&(gen_oneD[mu]), sizeof(double)*2*16*GK_localVolume, 
		      cudaHostAllocMapped)) != cudaSuccess)
      errorQuda("Error allocating memory gen_oneD\n");
    if((cudaHostAlloc(&(std_csvC[mu]), sizeof(double)*2*16*GK_localVolume, 
		      cudaHostAllocMapped)) != cudaSuccess)
      errorQuda("Error allocating memory std_csvC\n");
    if((cudaHostAlloc(&(gen_csvC[mu]), sizeof(double)*2*16*GK_localVolume, 
		      cudaHostAllocMapped)) != cudaSuccess)
      errorQuda("Error allocating memory gen_csvC\n");
  ...
  if( (buf_std_oneD = (double**) malloc(4*sizeof(double*)))==NULL ) 
    errorQuda("Allocation of buffer buf_std_oneD failed.\n");
  if( (buf_gen_oneD = (double**) malloc(4*sizeof(double*)))==NULL ) 
    errorQuda("Allocation of buffer buf_gen_oneD failed.\n");
  if( (buf_std_csvC = (double**) malloc(4*sizeof(double*)))==NULL ) 
    errorQuda("Allocation of buffer buf_std_csvC failed.\n");
  if( (buf_gen_csvC = (double**) malloc(4*sizeof(double*)))==NULL ) 
    errorQuda("Allocation of buffer buf_gen_csvC failed.\n");
  ...
  if(useTSM){  
    ...
    if((cudaHostAlloc(&std_uloc_LP, sizeof(double)*2*16*GK_localVolume, cudaHostAllocMapped)) != cudaSuccess)
      errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: Error allocating memory std_uloc_LP\n");
    if((cudaHostAlloc(&gen_uloc_LP, sizeof(double)*2*16*GK_localVolume, cudaHostAllocMapped)) != cudaSuccess)
      errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: Error allocating memory gen_uloc_LP\n");
    ...
    if(gen_oneD_LP == NULL) errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: Error allocating memory gen_oneD_LP higher level\n");
    if(std_oneD_LP == NULL) errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: Error allocating memory std_oneD_LP higher level\n");
    if(gen_csvC_LP == NULL) errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: Error allocating memory gen_csvC_LP higher level\n");
    if(std_csvC_LP == NULL) errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: Error allocating memory std_csvC_LP higher level\n");
    cudaDeviceSynchronize();
  
    for(int mu = 0; mu < 4 ; mu++){
      if((cudaHostAlloc(&(std_oneD_LP[mu]), sizeof(double)*2*16*GK_localVolume, cudaHostAllocMapped)) != cudaSuccess)
	errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: Error allocating memory std_oneD_LP\n");
      if((cudaHostAlloc(&(gen_oneD_LP[mu]), sizeof(double)*2*16*GK_localVolume, cudaHostAllocMapped)) != cudaSuccess)
	errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: Error allocating memory gen_oneD_LP\n");
      if((cudaHostAlloc(&(std_csvC_LP[mu]), sizeof(double)*2*16*GK_localVolume, cudaHostAllocMapped)) != cudaSuccess)
	errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: Error allocating memory std_csvC_LP\n");
      if((cudaHostAlloc(&(gen_csvC_LP[mu]), sizeof(double)*2*16*GK_localVolume, cudaHostAllocMapped)) != cudaSuccess)
	errorQuda("calcMG_loop_wOneD_TSM_EvenOdd: Error allocating memory gen_csvC_LP\n");    
  ...
  if(useTSM){
    ...
    for(int is = 0 ; is < Nrun ; is++){
      ...
      //-high-precision
      t1 = MPI_Wtime();
      oneEndTrick_w_One_Der<double>(*x,*tmp3,*tmp4,param, gen_uloc, 
				    std_uloc, gen_oneD, std_oneD, gen_csvC, std_csvC);
      t2 = MPI_Wtime();
      printfQuda("TIME_REPORT: NHP %04d - HP Contractions: %f sec\n",is+1,t2-t1);

      //-low-precision
      t1 = MPI_Wtime();
      oneEndTrick_w_One_Der<double>(*x_LP,*tmp3,*tmp4,param, gen_uloc_LP, 
				    std_uloc_LP, gen_oneD_LP, std_oneD_LP, 
				    gen_csvC_LP, std_csvC_LP); 
      t2 = MPI_Wtime();
	...
	if(GK_nProc[2]==1){
	  ...
	  copyLoopToWriteBuf(buf_std_uloc_HP,tmp_loop,iPrint,info.Q_sq,Nmoms,mom);
	  doCudaFFT_v2<double>(std_uloc_LP,tmp_loop);
	  copyLoopToWriteBuf(buf_std_uloc_LP,tmp_loop,iPrint,info.Q_sq,Nmoms,mom);

	  // dOp
	  doCudaFFT_v2<double>(gen_uloc   ,tmp_loop);
	  copyLoopToWriteBuf(buf_gen_uloc_HP,tmp_loop,iPrint,info.Q_sq,Nmoms,mom);
	  doCudaFFT_v2<double>(gen_uloc_LP,tmp_loop);
	  copyLoopToWriteBuf(buf_gen_uloc_LP,tmp_loop,iPrint,info.Q_sq,Nmoms,mom);
	  
	  for(int mu = 0 ; mu < 4 ; mu++){
	    // Loops
	    doCudaFFT_v2<double>(std_oneD[mu]   ,tmp_loop);
	    copyLoopToWriteBuf(buf_std_oneD_HP[mu],tmp_loop,iPrint,info.Q_sq,Nmoms,mom);
	    doCudaFFT_v2<double>(std_oneD_LP[mu],tmp_loop);
	    copyLoopToWriteBuf(buf_std_oneD_LP[mu],tmp_loop,iPrint,info.Q_sq,Nmoms,mom);
	    
	    // LoopsCv
	    doCudaFFT_v2<double>(std_csvC[mu]   ,tmp_loop);
	    copyLoopToWriteBuf(buf_std_csvC_HP[mu],tmp_loop,iPrint,info.Q_sq,Nmoms,mom); 
	    doCudaFFT_v2<double>(std_csvC_LP[mu],tmp_loop);
	    copyLoopToWriteBuf(buf_std_csvC_LP[mu],tmp_loop,iPrint,info.Q_sq,Nmoms,mom);

	    // LpsDw
	    doCudaFFT_v2<double>(gen_oneD[mu]   ,tmp_loop);
	    copyLoopToWriteBuf(buf_gen_oneD_HP[mu],tmp_loop,iPrint,info.Q_sq,Nmoms,mom); 
	    doCudaFFT_v2<double>(gen_oneD_LP[mu],tmp_loop);
	    copyLoopToWriteBuf(buf_gen_oneD_LP[mu],tmp_loop,iPrint,info.Q_sq,Nmoms,mom);

	    // LpsDwCv
	    doCudaFFT_v2<double>(gen_csvC[mu]   ,tmp_loop);
	    copyLoopToWriteBuf(buf_gen_csvC_HP[mu],tmp_loop,iPrint,info.Q_sq,Nmoms,mom); 
	    doCudaFFT_v2<double>(gen_csvC_LP[mu],tmp_loop);
	    copyLoopToWriteBuf(buf_gen_csvC_LP[mu],tmp_loop,iPrint,info.Q_sq,Nmoms,mom);
	  }
	}
	else if(GK_nProc[2]>1){
	  ...
	  for(int mu=0;mu<4;mu++){
	    performFFT<double>(buf_std_oneD_HP[mu], std_oneD[mu], iPrint, Nmoms, momQsq);
	    performFFT<double>(buf_std_oneD_LP[mu], std_oneD_LP[mu], iPrint, Nmoms, momQsq);
	    performFFT<double>(buf_std_csvC_HP[mu], std_csvC[mu], iPrint, Nmoms, momQsq);
	    performFFT<double>(buf_std_csvC_LP[mu], std_csvC_LP[mu], iPrint, Nmoms, momQsq);
	    performFFT<double>(buf_gen_oneD_HP[mu], gen_oneD[mu], iPrint, Nmoms, momQsq);
	    performFFT<double>(buf_gen_oneD_LP[mu], gen_oneD_LP[mu], iPrint, Nmoms, momQsq);
	    performFFT<double>(buf_gen_csvC_HP[mu], gen_csvC[mu], iPrint, Nmoms, momQsq);
	    performFFT<double>(buf_gen_csvC_LP[mu], gen_csvC_LP[mu], iPrint, Nmoms, momQsq);
	  }
	}
	t2 = MPI_Wtime();
