...
if (sid < c_threads/c_localL[3]){
	  ...
	  for(int c1 = 0 ; c1 < 3 ; c1++)
		...
		if( norm(gamma[ku][lu]) > 1e-4 ){
		  ...
		  if(c_dimBreak[dir] == true && (x_id[dir] == c_localL[dir]-1) )
		    bdf[ku][lu] = bdf[ku][lu] + PROP(seqTex,ku,pu,c1,c2) * GAUGE(gaugeTex,dir,c1,c3) * PROPplusSur(fwdTex,dir,lu,pu,c3,c2);
		  else
		    bdf[ku][lu] = bdf[ku][lu] + PROP(seqTex,ku,pu,c1,c2) * GAUGE(gaugeTex,dir,c1,c3) * PROPplusStr(fwdTex,dir,lu,pu,c3,c2);
		  // x x-dir x-dir  (minus) ====
		  if(c_dimBreak[dir] == true && x_id[dir] == 0)
		    bdf[ku][lu] = bdf[ku][lu] - PROP(seqTex,ku,pu,c1,c2) * conj(GAUGEminusSur(gaugeTex,dir,c3,c1)) * PROPminusSur(fwdTex,dir,lu,pu,c3,c2);
		  else
		    bdf[ku][lu] = bdf[ku][lu] - PROP(seqTex,ku,pu,c1,c2) * conj(GAUGEminusStr(gaugeTex,dir,c3,c1)) * PROPminusStr(fwdTex,dir,lu,pu,c3,c2);
		  // x+dir x x (minus) =======
		  if(c_dimBreak[dir] == true && (x_id[dir] == c_localL[dir]-1) )
		    bdf[ku][lu] = bdf[ku][lu] - PROPplusSur(seqTex,dir,ku,pu,c1,c2) *  conj(GAUGE(gaugeTex,dir,c3,c1)) * PROP(fwdTex,lu,pu,c3,c2);
		  else
		    bdf[ku][lu] = bdf[ku][lu] - PROPplusStr(seqTex,dir,ku,pu,c1,c2) *  conj(GAUGE(gaugeTex,dir,c3,c1)) * PROP(fwdTex,lu,pu,c3,c2);
		  // x-dir x-dir x (plus) ==== 
		  if(c_dimBreak[dir] == true && x_id[dir] == 0)
		    bdf[ku][lu] = bdf[ku][lu] + PROPminusSur(seqTex,dir,ku,pu,c1,c2) * GAUGEminusSur(gaugeTex,dir,c1,c3) * PROP(fwdTex,lu,pu,c3,c2);
		  else
		    bdf[ku][lu] = bdf[ku][lu] + PROPminusStr(seqTex,dir,ku,pu,c1,c2) * GAUGEminusStr(gaugeTex,dir,c1,c3) * PROP(fwdTex,lu,pu,c3,c2);
		}
    ...
    for(int iop = 0 ; iop < 16 ; iop++){
      ...
      for(int ku = 0 ; ku < 4 ; ku++)
	for(int lu = 0 ; lu < 4 ; lu++)
	  accum[iop] = accum[iop] + gamma[ku][lu] * bdf[ku][lu];
    }
