...
namespace quda {  
  ...
  MG::MG(MGParam &param, TimeProfile &profile_global)
    ...
    matCoarseResidual(nullptr), matCoarseSmoother(nullptr) {
	...
	if (param.mg_global.generate_all_levels == QUDA_BOOLEAN_YES || param.level == 0) {
      ...
      } else if (strcmp(param.mg_global.vec_infile,"")!=0) {
	// only load if infile is defined and not computing
	loadVectors(param.B);
      }
    }
    ...
    if (param.coarse_grid_solution_type == 
	QUDA_MATPC_SOLUTION && param.smoother_solve_type != QUDA_DIRECT_PC_SOLVE)
	...
	csParam.gammaBasis = param.level > 0 ? 
	  QUDA_DEGRAND_ROSSI_GAMMA_BASIS: QUDA_UKQCD_GAMMA_BASIS;
      }
      r = ColorSpinorField::Create(csParam);

      // if we're using preconditioning then allocate storate for the 
      // preconditioned source vector
	...
	param.mg_global.geo_block_size[param.level][i] = param.geoBlockSize[i];
      ...
      r_coarse = param.B[0]->CreateCoarse(param.geoBlockSize, 
					  param.spinBlockSize, 
					  param.Nvec, 
					  param.mg_global.location[param.level+1]);

      // create coarse solution vector
      x_coarse = param.B[0]->CreateCoarse(param.geoBlockSize, 
					  param.spinBlockSize, 
					  param.Nvec, 
					  param.mg_global.location[param.level+1]);

      // create coarse temporary vector
      tmp_coarse = param.B[0]->CreateCoarse(param.geoBlockSize, 
					    param.spinBlockSize, 
					    param.Nvec, 
					    param.mg_global.location[param.level+1]);
	...
	(param.coarse_grid_solution_type == QUDA_MATPC_SOLUTION && 
	 param.smoother_solve_type == QUDA_DIRECT_PC_SOLVE);
      
      // create coarse grid operator
      DiracParam diracParam;
      diracParam.transfer = transfer;

      diracParam.dirac = preconditioned_coarsen ? 
	const_cast<Dirac*>(param.matSmooth.Expose()) : 
	const_cast<Dirac*>(param.matResidual.Expose());
      ...
      diracParam.kappa *= param.mg_global.delta_kappaCG;
      //Mass is not set in original code... do we need to set it at all?
      diracParam.mass = 0.5/(diracParam.kappa) - 4.0;
      diracParam.mu = (param.mg_global.invert_param->mu) * (param.mg_global.delta_muCG);
      
      printfQuda("\n\n\n### QKXTM: DMH Experimental MG additions Coarse Grid Setup ###\n");
      printfQuda("Kappa = %e\n", diracParam.kappa);
      printfQuda("Mass = %e\n", diracParam.mass);
      printfQuda("Mu = %e\n", diracParam.mu);
      diracParam.Ls = 1;
      printfQuda("DiracParam print to check...\n");
      diracParam.print();
      printfQuda("\n\n");
      */
	...
	new DiracCoarse(static_cast<DiracCoarse&>(*diracCoarseResidual), diracParam);
      // for coarse grids these always alias for now 
      // (FIXME half precision support for coarse op)
      diracCoarseSmootherSloppy = diracCoarseSmoother;  
	...
	(*B_coarse)[i] = param.B[0]->CreateCoarse(param.geoBlockSize, 
						  param.spinBlockSize, param.Nvec);
	...
	for (int i=0; i<param.Nvec; i++) {
      ...
      param_coarse = new MGParam(param, *B_coarse, *matCoarseResidual, 
				 *matCoarseSmoother, *matCoarseSmootherSloppy, 
				 param.level+1);
	...
	if (param.mg_global.coarse_grid_solution_type[param.level+1] == QUDA_MATPC_SOLUTION) {
	  Solver *solver = Solver::create(*param_coarse_solver, 
					  *matCoarseSmoother, 
					  *matCoarseSmoother, 
					  *matCoarseSmoother, 
					  profile);
	  sprintf(coarse_prefix,"MG level %d (%s): ", param.level+2, 
		  param.mg_global.location[param.level+1] == QUDA_CUDA_FIELD_LOCATION ? 
		  "GPU" : "CPU" );
	  coarse_solver = new PreconditionedSolver(*solver, *matCoarseSmoother->Expose(), 
						   *param_coarse_solver, 
						   profile, coarse_prefix);
	} else {
	  Solver *solver = Solver::create(*param_coarse_solver, 
					  *matCoarseResidual, 
					  *matCoarseResidual, 
	  ...
	  sprintf(coarse_prefix,"MG level %d (%s): ", param.level+2, 
		  param.mg_global.location[param.level+1] == QUDA_CUDA_FIELD_LOCATION ? 
		  "GPU" : "CPU" );
	  coarse_solver = new PreconditionedSolver(*solver, *matCoarseResidual->Expose(), 
						   *param_coarse_solver, 
						   profile, coarse_prefix);
	}	
    ...
    //QKXTM: DMH Comment out verification step.
  ...
  void MG::reset() {
    QudaSiteSubset site_subset = param.coarse_grid_solution_type == QUDA_MATPC_SOLUTION ? 
      QUDA_PARITY_SITE_SUBSET : QUDA_FULL_SITE_SUBSET;
    QudaMatPCType matpc_type = param.mg_global.invert_param->matpc_type;
    QudaParity parity = ((matpc_type == QUDA_MATPC_EVEN_EVEN || 
			  matpc_type == QUDA_MATPC_EVEN_EVEN_ASYMMETRIC) ? 
			 QUDA_EVEN_PARITY : QUDA_ODD_PARITY);
    // use this to force location of transfer    
    transfer->setSiteSubset(site_subset, parity); 
  ...
  MG::~MG() {
	...
	delete B_coarse;
      ...
      if (diracCoarseSmootherSloppy && diracCoarseSmootherSloppy != diracCoarseSmoother) {
  ...
  void MG::operator()(ColorSpinorField &x, ColorSpinorField &b) {
    ...
    //printfQuda("outer_solution_type = %d, inner_solution_type = %d\n", outer_solution_type, inner_solution_type);
  ...
  void MG::generateNullVectors(std::vector<ColorSpinorField*> B) {
	...
	//QKXTM: DMH mu rescale
	printfQuda("MUBUG mu = %.6f\n", param.mg_global.invert_param->mu);
	(*solve)(*out, *in);
